...
router.get('/template/:lov_id', async (req, res) => {
  // Access the User ID from req.locals
  const partOfLists = await db.getUserCurentList(req.locals.userId, req.params.lov_id);
...
router.put('/template', async (req, res) => {
  try {
    const partOfLists = await db.getUserCurentList(req.locals.userId, req.body.lov_id);
    ...
    const updatedList = await db.updateListTemplate(req.locals.userId, req.body.lov_id, listForUpdate);
...
router.put('/template/change', async (req, res) => {
  try {
    ...
    const updatedList = await db.updateListTemplate(req.locals.userId, req.body.lov_id, newListForUpdate);
...
router.put('/template/delete', async (req, res) => {
  try {
    const updatedList = await db.updateListTemplate(req.locals.userId, req.body.lov_id, req.body.template);
    if (updatedList) {
